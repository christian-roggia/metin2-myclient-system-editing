...
TGAImg::TGAImg()
 {
  ...
  iWidth=iHeight=iBPP=bEnc=iID=0;
  lImageSize=0;
 }
...
void TGAImg::InvertImage()
{
    unsigned char *tempImage = (unsigned char *)malloc(iWidth * iHeight * (iBPP/8));
    int delta;

    memcpy(tempImage, pImage, iWidth * iHeight * (iBPP/8));
    ...
    for (int x = 0; x < iHeight; x++)
    {
        ...
        {
            delta = (iWidth - y) * 4;
            pImage[(x * iWidth + y) * 4 + 0] = tempImage[(iHeight * iWidth * 4) - delta - (x * iWidth * 4) + 0];
            pImage[(x * iWidth + y) * 4 + 1] = tempImage[(iHeight * iWidth * 4) - delta - (x * iWidth * 4) + 1];
            pImage[(x * iWidth + y) * 4 + 2] = tempImage[(iHeight * iWidth * 4) - delta - (x * iWidth * 4) + 2];
            pImage[(x * iWidth + y) * 4 + 3] = tempImage[(iHeight * iWidth * 4) - delta - (x * iWidth * 4) + 3];
        }
    }
}
